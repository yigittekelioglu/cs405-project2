...
class MeshDrawer {
	...
	constructor() {
		...
		this.locationOfEnableLighting = gl.getUniformLocation(this.prog, 'enableLighting');
		this.locationOfLightPosition = gl.getUniformLocation(this.prog, 'lightPos');
        this.locationOfAmbience = gl.getUniformLocation(this.prog, 'ambient');
		this.locationOfNormal = gl.getAttribLocation(this.prog, 'normal');
	...
	draw(trans) {
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, this.buffer);
        gl.enableVertexAttribArray(this.locationOfNormal);
        gl.vertexAttribPointer(this.locationOfNormal, 3, gl.FLOAT, false, 0, 0);
		gl.uniform3fv(this.locationOfLightPosition, [lightX, lightY, 5.0]);
	...
	enableLighting(show) {
		...
		gl.useProgram(this.prog);
		gl.uniform1i(this.locationOfEnableLighting, show);
	}
	...
	setAmbientLight(ambient) {
		...
		gl.useProgram(this.prog);
    	gl.uniform1f(this.locationOfAmbience, ambient);
...
const meshFS = `
	...
	void main()
	{
		...
		if(showTex && enableLighting){
			...
			vec3 diffuseColor = diff * texture2D(tex, v_texCoord).rgb;

			vec3 lighting = ambientColor + diffuseColor;
			gl_FragColor = vec4(lighting,texture2D(tex, v_texCoord).a);
		}
		else if(showTex){
